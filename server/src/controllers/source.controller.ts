...
export async function listSources(
  ...
  const sources =
    await youtubeSyncStore.listSourceAccounts();
...
export async function analyzeSource(
  ...
  const body =
    req.body as Partial<AnalyzeSourceRequest>;
  ...
  const result =
    await sourceService.analyzeUrl(body.url);
  ...
  if (
    ...
    const sync =
      await youtubeSyncService.syncUrl(body.url);

    res
      .status(sync.status === 'failed' ? 422 : 200)
      .json({
        ...result,
        sync,
        metadataMessage:
          result.metadataMessage ?? sync.message,
      });
